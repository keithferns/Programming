...
@synthesize schedulerPopover, reminderPopover, navPopover;
@synthesize datePicker, timePicker;
@synthesize theAppointment, theTask;
@synthesize addField;
...
- (void) addNewAppointment{
    ...
    [theAppointment setCreationDate:[NSDate date]];
    [theAppointment setType:[NSNumber numberWithInt:1]];
    
    //tableViewController = [[AppointmentsTableViewController alloc]init];
    
    myData.selectedAppointment = theAppointment;
    [self setEditingView];
    [self cancelPopover:nil];
...
- (void) addNewTask{
    ...
    [theTask setCreationDate:[NSDate date]];
    [theTask setType:[NSNumber numberWithInt:2]];
...
- (void) doneAction{
    ...
    if ([myData.noteType intValue]==1) {
        theAppointment.text = textView.text;
        NSString *selectedDate = [dateFormatter stringFromDate:theAppointment.doDate];
        NSString *selectedStart = [timeFormatter stringFromDate:theAppointment.doTime];
        NSString *selectedEnd = [timeFormatter stringFromDate:theAppointment.endTime];
        NSMutableString *text = [NSMutableString stringWithFormat:@"Scheduled Date: %@\nStarts At: %@. Ends At: %@\n\n%@",selectedDate, selectedStart, selectedEnd, theAppointment.text];
        textView.text = text;        
        if (theAppointment.doDate == nil || theAppointment.text == @""){
        ...
        theTask.text = textView.text;
        if (theTask.doDate == nil || theTask.text == @""){
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Warning" message:@"No Appointment Date or Text" delegate:self cancelButtonTitle:@"OK" otherButtonTitles: nil];
            [alert show];
            [alert release];
            return;
...
- (void) toggleCancel:(id) sender {
    if (self.navigationItem.leftBarButtonItem == nil){
        ...
        [barbutton release];
        [cancelButton addTarget:self action:@selector(clearAll) forControlEvents:(UIControlEventTouchDown)];
    }
}
...
-(void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row
    ...
    MyDataObject *myData = [self myDataObject];
    if ([myData.noteType intValue] == 1) {
        theAppointment.recurring = [recurring objectAtIndex:row];
    }
    else {
        theTask.recurring = [recurring objectAtIndex:row];
    }
    [[NSNotificationCenter defaultCenter] postNotificationName:@"PopOverShouldUpdateNotification"object:theAppointment userInfo:inputObjects];
...
- (void) displaySelectedRow:(NSNotification *) notification {
    ...
    if ([[notification object] isKindOfClass:[Appointment class]]) {
        ...
        NSLog(@"THE SELECTED NOTIFICATION OBJECT IS AN TASK");
    ...
    [self toggleEdit:nil];   
    [self toggleAddNew:nil];
    NSLog(@"In displaySelectedRow Method:CALENDAR VIEW");
    return;
}
...
- (void)datePickerChanged:(id)sender{
   ...
   // [dateComponents setDay:[dateComponents day]];
    ...
    if (theAppointment != nil){
        theAppointment.doDate = selectedDate;
    }
    else if (theTask != nil){
        theTask.doDate = selectedDate;
    }
...
- (void) setStartTime:(id)sender{
    ...
    //CASE: the popOver in view, TIMEPICKER time selected and  STARTTIME button tapped -> 
        ...
        if ([myData.noteType intValue]== 1){
            ...
            [timeComponents setMinute:[timeComponents minute]];
            [timeComponents setSecond:[timeComponents second]];
            theAppointment.doTime = selectedTime;
        }
        else {
...
- (void) setEndTime:(id)sender{
    ...
    //CASE: popOver in view, TIMEPICKER time selected and the ENDTIME button is tapped --> 
        ...
        if ([myData.noteType intValue]== 1){
            theAppointment.endTime = selectedTime;
        }
        else {
            [timeComponents setHour:0];
            [timeComponents setMinute:0];
