...
@synthesize recurring, addField, calendarView, theItem,dateFormatter, timeFormatter;
...
- (void) viewWillAppear:(BOOL)animated{
    ...
    //Initialize the table view controller. But do not add the tableview
    ...
    //Animate  bringing in the table View
    {
	...
	[tableViewController.tableView deselectRowAtIndexPath:tableSelection animated:NO];
        NSLog(@"Current View Controller Subviews: %@", [self.view subviews]);}//PP
}
...
- (void) addNewEvent:(id)sender {
    ...
    if ([sender tag]==1) {
        [theItem createNewItem:textView.text ofType:[NSNumber numberWithInt:1]];//Method in NewItemOrEvent inserts a new Note object into adding MOC and sets the text attribute to textView.text and sets the type to Appointment
        return;
    }
    else {
        [theItem createNewItem:textView.text ofType:[NSNumber numberWithInt:2]];//Method in NewItemOrEvent inserts a new Note object into adding MOC and sets the text attribute to textView.text and sets the type to Task
        return;
    }
}
...
- (void) saveMemo:(id)sender {    
    ...
    theItem.addingContext = addingContext;
    [theItem createNewItem:textView.text ofType:[NSNumber numberWithInt:0]];    
    [theItem saveNewItem];
...
- (void) doneAction{
    [self toggleLeftBarButtonItem:nil];
    [self toggleRightBarButtonItem:nil];
    theItem.theNote.text = textView.text; // set the text for the new note to current value of the textView
    NSString *selectedDate = [dateFormatter stringFromDate:theItem.theNote.doDate]; 
    NSString *selectedStart = [timeFormatter stringFromDate:theItem.theNote.doTime];
    NSString *selectedEnd = [timeFormatter stringFromDate:theItem.theNote.endTime];
    [theItem saveNewItem]; //save the newNote as Appointment, task or memo. delete newNote.
    if (theItem.theAppointment != nil) {
        if (theItem.theAppointment.doDate == nil || theItem.theAppointment.text == @""){
     ...
     else if (theItem.theTask != nil) {
        if (theItem.theTask.doDate == nil || theItem.theTask.text == @""){
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Warning" message:@"No Appointment Date or Text" delegate:self cancelButtonTitle:@"OK" otherButtonTitles: nil];
            [alert show];
            [alert release];
            //return user to the missing input view. if text is missing, make textView first responder etc.
...
-(void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row
    ...
    theItem.recurring = [recurring objectAtIndex:row];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"PopOverShouldUpdateNotification" object:theItem.recurring userInfo:inputObjects];
}
...
- (void)datePickerChanged:(id)sender{
    ...
    if (theItem.theNote != nil){
        NSLog(@"method:datePickerChanged. newItem.newNote is not nil");
        [theItem updateSelectedDate:selectedDate];
    }
...
- (void) setStartTime:(id)sender{
    ...
    //CASE: the popOver in view, TIMEPICKER time selected and  STARTTIME button tapped -> 
        ...
        if ([myData.noteType intValue]== 1){
            ...
            [timeComponents setMinute:[timeComponents minute]];
            [timeComponents setSecond:[timeComponents second]];
            theItem.theNote.doTime = selectedTime;
        }
        else {
...
- (void) addEndTimeField{
    if ([schedulerPopover isPopoverVisible]) {
        ...
        NSArray *keys = [NSArray arrayWithObjects:@"picker", @"toolbar", @"num",nil];
        NSDictionary *inputObjects = [NSDictionary dictionaryWithObjects:objects forKeys:keys];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"PopOverShouldUpdateNotification"object:theItem.theNote userInfo:inputObjects];
    }
}
- (void) setEndTime:(id)sender{
    ...
    //CASE: popOver in view, TIMEPICKER time selected and the ENDTIME button is tapped --> 
        ...
        if ([myData.noteType intValue]== 1){
            theItem.theNote.endTime = selectedTime;
        }
        else {
            [timeComponents setHour:0];
            [timeComponents setMinute:0];
...
- (void) addRecurringField{
    if ([schedulerPopover isPopoverVisible]) {
        ...
        NSArray *keys = [NSArray arrayWithObjects:@"picker", @"toolbar", @"num",nil];
        NSDictionary *inputObjects = [NSDictionary dictionaryWithObjects:objects forKeys:keys];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"PopOverShouldUpdateNotification"object:theItem userInfo:inputObjects];
    }
}
